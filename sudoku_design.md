...
- **난이도 선택 (Difficulty Selection - Blue Gradient Design)**:
  - 정보 바 좌측에 **블루 그라데이션 필(Pill) 형태의 난이도 버튼** 제공 (`Colors.blue.shade600` ~ `Colors.indigo.shade600`).
  - 클릭 시 라운딩 처리된 **연푸른색 팝업 카드 (`Colors.blue.shade50`)**에 아이콘(😊, 😐, 🔥)과 체크마크가 포함된 블루 테마 메뉴 출력.
  - **초급 (Easy)**: 빈칸 30개
  - **중급 (Medium)**: 빈칸 42개
  - **고급 (Hard)**: 빈칸 52개
...
## 3. 아키텍처 및 파일 구조 (Project Architecture)
...
- `android/settings.gradle.kts`: Gradle/AGP 환경 변수(`ANDROID_PREFS_ROOT`) 자동 정리 스크립트.
...
- **v1.1.0** (2026-09-02):
  - `+ New Game` 버튼 추가.
  - 난이도 선택(초급/중급/고급) 기능 및 **블루 그라데이션 팝업 UI 디자인** 개선.
  ...
  - Android Studio Gradle 환경 변수 빌드 호환성 개선.
